...
const all={invoice:true,fields_match:true,total_includes_vat:true};
async function main(){
  ...
  await assert.rejects(review(treasurer,r,'approve',{...all,invoice:false}));
  await assert.rejects(review(treasurer,r,'approve',{...all,total_includes_vat:false}));
  ...
  r=await review(cfo,r);assert.equal(r.status,'APPROVED');
  const logs=await applicant.audit(r.id);assert.equal(logs.length,3);assert.equal(logs[0].actor_role,'cfo');
  r=await submit(5000000);r=await review(treasurer,r,'clarify',{},'TEST: bổ sung PDF hoặc sửa trường hóa đơn cho khớp form.');assert.equal(r.status,'NEEDS_INFO');
  ...
  console.log('PASS: role isolation, private evidence, manual invoice checks, 20m boundary, race, escalation, supplementation and audit.');
