...
const STORE_KEY = 'finref-demo-state-v2';
...
const seed = [
  { requestId: 'PAY-0318', requester: 'Nguyễn Minh An', department: 'Marketing', amount: 12500000, vendor: 'Công ty In Sao Mai', purpose: 'In ấn tài liệu sự kiện', invoiceNumber: 'INV-2026-0318', status: 'TREASURER_REVIEW', code: 'CLEAR', createdAt: '20/09/2026 · 09:14', hasStamp: true, hasSignature: true, hasAcceptance: true },
...
function evaluate(data) {
  ...
  if (missing.length) return { kind: 'escalate', code: 'U1', title: 'Thiếu dữ kiện', receiver: 'Thủ quỹ', reason: `Còn thiếu ${missing.length} trường bắt buộc.`, question: 'Bổ sung thông tin còn thiếu trước khi gửi?', status: 'NEEDS_INFO' };
  if (!data.hasStamp || !data.hasSignature || !data.hasAcceptance) {
    const gaps = [!data.hasStamp && 'dấu đỏ', !data.hasSignature && 'chữ ký', !data.hasAcceptance && 'biên bản nghiệm thu'].filter(Boolean);
    return { kind: 'escalate', code: 'U1', title: 'Thiếu minh chứng', receiver: 'Thủ quỹ', reason: `Chưa xác minh được: ${gaps.join(', ')}.`, question: `Hồ sơ thiếu ${gaps.join(' và ')}. Bổ sung minh chứng trước khi xử lý?`, status: 'NEEDS_INFO' };
  }
  if (data.policyIssue || data.budgetStatus === 'over') return { kind: 'reject', code: 'U2', title: 'Vi phạm policy', receiver: 'Thủ quỹ', reason: data.policyIssue ? 'Khoản chi ngoài policy hiện hành.' : 'Khoản chi vượt ngân sách được cấp.', question: 'Có cần xin ngoại lệ theo policy không?', status: 'REJECTED' };
  if (data.amount > 20000000) return { kind: 'escalate', code: 'U3', title: 'Vượt thẩm quyền', receiver: 'Giám đốc Tài chính', reason: `Số tiền ${money(data.amount)} vượt ngưỡng 20.000.000 ₫.`, question: 'Chuyển hồ sơ cho Giám đốc Tài chính phê duyệt?', status: 'CFO_REVIEW' };
  return { kind: 'ready', code: 'CLEAR', title: 'Đã gửi thủ quỹ', receiver: 'Thủ quỹ', reason: 'Hồ sơ đủ minh chứng và nằm trong hạn mức thủ quỹ.', question: 'Hồ sơ đã vào hàng đợi thủ quỹ để kiểm tra.', status: 'TREASURER_REVIEW' };
}
...
function renderAudit() {
  const entries = [...state.audits, { title: 'Policy FIN-2026 / v1.0', subtitle: 'Rules Engine · local demo', hash: 'policy' }].slice(0, 8);
...
function renderDecision(item) {
  ...
  const receiver = currentRole === 'cfo' ? 'Giám đốc Tài chính' : currentRole === 'treasurer' ? 'Thủ quỹ' : 'Người nộp đơn';
  decisionResult.innerHTML = `<div class="decision-banner ${resultKind}"><div class="decision-icon">${resultKind === 'ready' ? '✓' : resultKind === 'reject' ? '×' : '!'}</div><div><strong>${escapeHtml(label)}</strong><small>${escapeHtml(item.code || 'U1')} · ${escapeHtml(receiver)}</small></div></div><div class="decision-meta"><div class="meta-row"><span>Mã hồ sơ</span><strong>${escapeHtml(item.requestId)}</strong></div><div class="meta-row"><span>Số tiền</span><strong>${money(item.amount)}</strong></div><div class="meta-row"><span>Nhà cung cấp</span><strong>${escapeHtml(item.vendor)}</strong></div><div class="meta-row"><span>Trạng thái</span><strong>${escapeHtml(label)}</strong></div></div><div class="question-box"><span class="eyebrow">GỢI Ý XỬ LÝ</span><p>${item.status === 'NEEDS_INFO' ? 'Vui lòng bổ sung minh chứng giấy có dấu đỏ và chữ ký.' : 'Chọn hành động phù hợp với thẩm quyền của bạn.'}</p><small class="reason-copy">Dữ liệu đang được lưu trong demo trên trình duyệt.</small></div><div class="decision-actions">${actionButtons(item)}</div>`;
...
document.querySelector('#verifyButton')?.addEventListener('click', (event) => { if (currentRole === 'applicant') { alert('Demo mô phỏng: checkbox là dữ liệu giả lập, chưa đọc hoặc lưu minh chứng. Đăng nhập trang chính để dùng luồng nhiều người.'); return; } const results = FinRefRules.verify().slice(0,5); const passed = results.filter(r=>r.pass).length; event.currentTarget.textContent = `Verify: ${passed}/5 PASS`; addAudit(`Kiểm tra logic mẫu · ${passed}/5 ca đạt`, now()); });
