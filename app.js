...
const roles={applicant:'Người nộp đơn',treasurer:'Thủ quỹ',cfo:'Giám đốc Tài chính'};
...
const checkLabels={paper:'Đã kiểm tra đúng hóa đơn giấy',stamp:'Đã kiểm tra dấu đỏ',signature:'Đã kiểm tra chữ ký',match:'Thông tin, số tiền và minh chứng khớp nhau',budget:'Khoản chi trong ngân sách được cấp',policy:'Khoản chi phù hợp chính sách'};
...
function resetForm() {
  ...
  $('formTitle').textContent='Tạo đề nghị mới'; $('submitButton').textContent='Gửi đến thủ quỹ →';
}
...
async function openRequest(id) {
  ...
  if($('supplementButton')) $('supplementButton').onclick=()=>{
    ...
    $('formTitle').textContent='Bổ sung PAY-'+r.id.slice(0,8); $('submitButton').textContent='Gửi lại đến thủ quỹ →';
...
async function review(r,action) {
  ...
  if(action==='approve'&&profile.role==='treasurer') {
    const result=FinRefRules.assess(r.amount,checks);
    if(result==='U1'||result==='U2') {notify(result+': chưa đủ xác nhận. Hãy kiểm tra minh chứng hoặc yêu cầu bổ sung/từ chối.',true);return;}
  }
  setBusy(true);
...
$('paymentForm').onsubmit=async event=>{
  ...
  try {
    ...
    resetForm(); notify('Đã gửi hồ sơ đến thủ quỹ.');
